...
const People = () => {
  ...
  useEffect(() => {
    // Save people data to local storage whenever it changes
  ...
  const handleImageChange = (e) => {
    const files = Array.from(e.target.files);
    setFormData({ ...formData, images: files.map((file) => URL.createObjectURL(file)) });
  };
  ...
  const filteredPeople = people.filter((person) =>
    `${person.first_name} ${person.last_name}`.toLowerCase().includes(searchQuery)
  );
